...
class Grid():
    ...
    def uptake(self,pulse,day):
        ...
        is_mineral = (self.Monomers.index == "NH4") | (self.Monomers.index == "PO4")
        ...
        #Input_Org = self.Monomer_ratios[is_org].mul(self.MonInput[is_org].tolist(),axis=0)
        Input_Mineral = self.Monomer_ratios[is_mineral].mul((self.MonInput[is_mineral]).tolist(),axis=0)
    ...
    def repopulation(self,output,day,mic_reinit):
        ...
        print("Reached repopulation function for day:", day)
        ...
        if day == 1094:
            ...
            self.Substrates_init = expand(pd.read_csv('initial_substrates_cellulose.csv', header=0,index_col=0).astype('float32'),self.gridsize)
            #self.Monomers_init   = expand(pd.read_csv('sub_mon_inputs_cellulose.csv', header=0,index_col=0).astype('float32'),self.gridsize)
            self.Monomers   = self.Monomers_init.copy(deep=True)
